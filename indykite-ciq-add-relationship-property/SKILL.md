...
- A **Service Account token** with Config API access — used to *create* the policy and Knowledge Query.
...
curl -X POST "$API_URL/configs/v1/authorization-policies" \
  ...
  -d @indykite-ciq-add-relationship-property/assets/policy-annotate-played-at.json
```

A `201 Created` returns the policy's `id` (GID). Save it — the Knowledge Query references it.
...
curl -X POST "$API_URL/configs/v1/knowledge-queries" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SERVICE_ACCOUNT_TOKEN" \
  -d @indykite-ciq-add-relationship-property/assets/knowledge-query-annotate-played-at.json
```
