#!/usr/bin/env bash
# capture.sh - batch upsert nodes via POST /capture/v1/nodes (Capture API).
#
# Required env vars:
...
#             assets/nodes-vehicle-rental.json. Use "-" for stdin.
#
# Usage:
#   ./capture.sh assets/nodes-vehicle-rental.json
#   cat body.json | ./capture.sh -
#   ./capture.sh --print assets/nodes-vehicle-rental.json   # print the curl (token redacted), don't run it

set -euo pipefail

ENDPOINT="/capture/v1/nodes"
