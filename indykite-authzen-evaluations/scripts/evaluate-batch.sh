#!/usr/bin/env bash
# evaluate-batch.sh - ask for many KBAC decisions in one call via
# POST /access/v1/evaluations (AuthZEN).
#
# Required env vars:
...
#
# Arguments:
#   $1            Path to a JSON file with the batch request body
#                 (top-level defaults + an evaluations[] array), e.g.
#                 assets/evaluations-provision-servers.json. Use "-" for stdin.
#
# Usage:
#   ./evaluate-batch.sh assets/evaluations-provision-servers.json
#   cat req.json | ./evaluate-batch.sh -
#   ./evaluate-batch.sh --print assets/evaluations-provision-servers.json   # print the curl (tokens redacted), don't run it
...
if [[ "${#}" -ne 1 ]]; then
    printf 'usage: %s [--print] <evaluations-request.json | ->\n' "${0}" >&2
    exit 2
fi
