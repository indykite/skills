...
|:------|:-------------|:---------------|
| [`indykite-agent-gateway`](indykite-agent-gateway/SKILL.md) | Deploy and configure IAG in front of an A2A agent or MCP server to enforce caller, workflow, and delegation-chain checks — with the optional IndyKite Token Service for multi-hop delegation (`X-IK-Token`). | "Put IAG in front of my MCP server so every call is authorized before it reaches it, and wire up the workflow in the IKG." |
...
| [`indykite-authzen-list-policies`](indykite-authzen-list-policies/SKILL.md) | List the project's ACTIVE KBAC policies at runtime from an AppAgent (`GET /access/v1/policies`, `ReadAuthZConfigs` permission) — each policy as stored, with tags, optionally filtered by `subject_type`. | "Which KBAC policies apply to `Person` right now, and which tags do they carry?" |
...
| [`indykite-ciq-whoami`](indykite-ciq-whoami/SKILL.md) | Resolve an end-user token to the IKG subject it acts as (`GET /contx-iq/v1/whoami`) — the `type` / `id` pair to use as `subject` in AuthZEN and CIQ requests. | "Which IKG node does this user's access token map to?" |
