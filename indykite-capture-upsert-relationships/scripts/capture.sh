#!/usr/bin/env bash
# capture.sh - batch upsert relationships via POST /capture/v1/relationships (Capture API).
#
# Required env vars:
...
#             assets/relationships-vehicle-rental.json. Use "-" for stdin.
#
# Usage:
#   ./capture.sh assets/relationships-vehicle-rental.json
#   cat body.json | ./capture.sh -
#   ./capture.sh --print assets/relationships-vehicle-rental.json   # print the curl (token redacted), don't run it

set -euo pipefail

ENDPOINT="/capture/v1/relationships"
