#!/usr/bin/env bash
# capture.sh - batch delete node properties via POST /capture/v1/nodes/properties/delete (Capture API).
#
# Required env vars:
...
#             assets/delete-node-properties.json. Use "-" for stdin.
#
# Usage:
#   ./capture.sh assets/delete-node-properties.json
#   cat body.json | ./capture.sh -
#   ./capture.sh --print assets/delete-node-properties.json   # print the curl (token redacted), don't run it

set -euo pipefail

ENDPOINT="/capture/v1/nodes/properties/delete"
