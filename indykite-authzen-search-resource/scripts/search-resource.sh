#!/usr/bin/env bash
# search-resource.sh - list the resources a subject may act on under a given
# action via POST /access/v1/search/resource (AuthZEN).
#
# Required env vars:
...
#                 (subject / resource.type / action / optional context), e.g.
#                 assets/search-resource-request.json. Use "-" for stdin.
#
# Usage:
#   ./search-resource.sh assets/search-resource-request.json
#   cat req.json | ./search-resource.sh -
#   ./search-resource.sh --print assets/search-resource-request.json   # print the curl (tokens redacted), don't run it
