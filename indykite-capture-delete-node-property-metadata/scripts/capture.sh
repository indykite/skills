#!/usr/bin/env bash
# capture.sh - batch delete node property metadata via POST /capture/v1/nodes/properties/metadata/delete (Capture API).
#
# Required env vars:
...
#             assets/delete-property-metadata.json. Use "-" for stdin.
#
# Usage:
#   ./capture.sh assets/delete-property-metadata.json
#   cat body.json | ./capture.sh -
#   ./capture.sh --print assets/delete-property-metadata.json   # print the curl (token redacted), don't run it

set -euo pipefail

ENDPOINT="/capture/v1/nodes/properties/metadata/delete"
