#!/usr/bin/env bash
# search-subject.sh - list the subjects allowed a given action on a resource via
# POST /access/v1/search/subject (AuthZEN).
#
# Required env vars:
...
#                 (subject.type / resource / action / optional context), e.g.
#                 assets/search-subject-request.json. Use "-" for stdin.
#
# Usage:
#   ./search-subject.sh assets/search-subject-request.json
#   cat req.json | ./search-subject.sh -
#   ./search-subject.sh --print assets/search-subject-request.json   # print the curl (tokens redacted), don't run it
