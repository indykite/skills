...
2. **Triple matches the policy?**
   ...
   - `action.name` is one of the policy's `actions` (case-sensitive - `CAN_BUY` ≠ `can_buy`).
   - `resource.type` equals the policy's `resource.type`.
3. **Reserved variable names.** The condition must bind variables literally named `subject` and `resource`. If the Cypher uses other names (`p`, `car`), the request's subject/resource never bind and the condition matches nothing.
...
5. **Partial parameters supplied and typed.** Every `$name` in the condition must appear in `context.input_params` (without the `$`). A numeric comparison (`<= $max_price`) needs a number, not a string - `"150000"` may not compare as expected.
...
- Malformed JSON, or `input_params` is missing a parameter the condition needs. The condition's `$name` set and the `input_params` keys must line up exactly.
