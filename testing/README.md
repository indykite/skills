...
This confirms each `execute.sh` (and the MCP `init-session.sh`) constructs a well-formed `curl` command — without sending it. Useful for verifying the script logic in isolation.
